...
# Reverting ad_keywords to be hardcoded as in the original setup
ad_keywords = [
...
# Function to save results to CSV for each ad-blocker
...
websites_file_path = "/Users/anish/Documents/Privacy/Privacy Project/websites_machine_1.txt"  # Replace this path as needed to specify which file to use
websites = []
try:
...
if __name__ == "__main__":
    for path in paths:
        with Pool(processes=4) as pool:  # Run with 4 parallel processes
            args = [(path, website) for website in websites]
        ...
        save_to_csv(extension_results, path.split('/')[-1].split('.')[0] if platform.system() == "Darwin" else path.split('\\')[-1].split('.')[0])
